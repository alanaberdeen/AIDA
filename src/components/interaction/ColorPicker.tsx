            ...
            <Popover.Panel className="absolute z-10 w-48 p-2 transform -translate-x-56 -translate-y-1/2">
              <div className="overflow-hidden rounded-md shadow-lg ring-1 ring-black ring-opacity-5">
                <div className="bg-white p-2">
                  <div className="text-sm font-medium text-gray-800"> Templates </div>
                  <div className="grid grid-cols-5 mt-2">
                    {templateColors.map((item) => (
                      <div
                        className={`${color.fill === item.fill ? "bg-gray-200": "" } rounded-sm flex justify-center align-middle`}
                        key={item.hex}  
                      >
                        <button
                ...
                <div className=" p-2 bg-white border-t-gray-50">
                  <div className="relative text-sm font-medium text-gray-800"> Custom </div>
                    <Popover>
                      ...
                      >
                        <Popover.Panel className="absolute z-10 transform -translate-x-full -mx-4 -translate-y-1/2">
                          <ChromePicker
                            color={{r: color.stroke[0], g: color.stroke[1], b: color.stroke[2], a: color.stroke[3]}}
                            onChange={(newColor) => {
                              onChange(color.fill, [newColor.rgb.r, newColor.rgb.g, newColor.rgb.b, newColor.rgb.a])
                            }}
                          />
                        </Popover.Panel>
                      </Transition>
                    ...
                    <Popover>
                      ...
                      >
                        <Popover.Panel className="absolute z-10 transform -translate-x-full -mx-4 -translate-y-1/2">
                          <ChromePicker
                            color={{r: color.fill[0], g: color.fill[1], b: color.fill[2], a: color.fill[3]}}
                            onChange={(newColor) => {
                              onChange([newColor.rgb.r, newColor.rgb.g, newColor.rgb.b, newColor.rgb.a], color.stroke)
                            }}
                          />
                        </Popover.Panel>
                      </Transition>
