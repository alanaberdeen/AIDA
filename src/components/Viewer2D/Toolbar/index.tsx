...
const Toolbar = (props: { map: Map }) => {
  ...
  const tools = ['pan', 'select', 'modify', 'point', 'lineString', 'polygon', 'box']
  ...
  useEffect(() => {
    ...
    const conditionalDragPan = new DragPan({ condition: (e) => e.originalEvent.button === 1 })
    ...
    const dragBox = new DragBox({ className: 'border border-blue-500 bg-gray-50 bg-opacity-25' });
    dragBox.on('boxend', () => {
  ...
  useEffect(() => {
    ...
    switch (activeTool) {
      case 'pan': activeInteractions.push('pan', 'rotate'); break
      case 'select': activeInteractions.push('select', 'translate', 'dragBox'); break
      case 'modify': activeInteractions.push('modify', 'select', 'dragBox'); break
      case 'point': activeInteractions.push('point'); break
      case 'lineString': activeInteractions.push('lineString'); break
      case 'polygon': activeInteractions.push('polygon'); break
      case 'box': activeInteractions.push('box'); break
      default: activeInteractions.push('pan'); break
    }
  ...
  return (
    <div className="absolute left-0 top-0 z-20 flex flex-col m-1 bg-gray-100 rounded-lg pointer-events-auto border border-gray-300">
      { tools?.includes('pan')
        && (
        <button
          className={`${activeTool === 'pan' ? 'text-blue-800 bg-gray-200' : 'text-gray-500'} block border-b rounded-t-lg p-2 text-sm leading-5 font-medium hover:bg-gray-50 active:bg-gray-100 transition ease-in-out duration-150`}
          onClick={() => setActiveTool('pan')}
          aria-label="Activate pan tool"
          title="Pan"
          type="button"
        >
          <svg viewBox="0 0 20 20" fill="currentColor" className="cursor-click w-4 h-4">
      ...
      { tools?.includes('select')
        && (
        <button
          className={`${activeTool === 'select' ? 'text-blue-800 bg-gray-200' : 'text-gray-500'} block p-2 text-sm leading-5 font-medium hover:bg-gray-50 active:bg-gray-100 transition ease-in-out duration-150`}
          onClick={() => setActiveTool('select')}
          aria-label="Activate select tool"
          title="Select"
          type="button"
        >
          <svg viewBox="0 0 24 24" fill="currentColor" className="cursor-click w-4 h-4">
      ...
      { tools?.includes('modify')
        && (
        <button
          className={`${activeTool === 'modify' ? 'text-blue-800 bg-gray-200' : 'text-gray-500'} block border-b p-2 text-sm leading-5 font-medium hover:bg-gray-50 active:bg-gray-100 transition ease-in-out duration-150`}
      ...
      { tools?.includes('point')
        && (
        <button
          className={`${activeTool === 'point' ? 'text-blue-800 bg-gray-200' : 'text-gray-500'} block border-b p-2 text-sm leading-5 font-medium hover:bg-gray-50 active:bg-gray-100 transition ease-in-out duration-150`}
          onClick={() => setActiveTool('point')}
          aria-label="Activate point tool"
          title="Point"
          type="button"
        >
          <svg viewBox="0 0 24 24" fill="currentColor" stroke="currentColor" className="cursor-click w-4 h-4">
      ...
      { tools?.includes('lineString')
        && (
        <button
          className={`${activeTool === 'lineString' ? 'text-blue-800 bg-gray-200' : 'text-gray-500'} block border-b p-2 text-sm leading-5 font-medium hover:bg-gray-50 active:bg-gray-100 transition ease-in-out duration-150`}
      ...
      { tools?.includes('polygon')
        && (
        <button
          className={`${activeTool === 'polygon' ? 'text-blue-800 bg-gray-200' : 'text-gray-500'} block p-2 text-sm leading-5 font-medium hover:bg-gray-50 active:bg-gray-100 transition ease-in-out duration-150`}
      ...
      { tools?.includes('box')
        && (
        <button
          className={`${activeTool === 'box' ? 'text-blue-800 bg-gray-200' : 'text-gray-500'} block p-2 rounded-b-lg text-sm leading-5 font-medium hover:bg-gray-50 active:bg-gray-100 transition ease-in-out duration-150`}
