...
const ActiveLayerControls = (props: { activeLayer: VectorLayer<VectorSource<Geometry>> }) => {
  ...
  useEffect(() => {
    const listener = () => { setOpacity(activeLayer.getOpacity()) }
    activeLayer.on('change:opacity', listener);
    return () => { activeLayer.un('change:opacity', listener) }
  }, [activeLayer])
