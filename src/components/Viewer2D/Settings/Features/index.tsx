...
const Features = (props: { map: Map }) => {
  ...
  const [featureClasses, setFeatureClasses]: [IFeatureClass[], Dispatch<IFeatureClass[]>] = useState(map.get('featureClasses') || []);
  const [activeFeatureClass, setActiveFeatureClass]: [IFeatureClass, Dispatch<IFeatureClass>] = useState(map.get('featureClasses')[0]);
      ...
      {({ open }) => (
        <>
          ...
          <Disclosure.Panel className="bg-white rounded-b-sm relative">
            ...
            {activeFeatureClass && <ActiveClassControls activeFeatureClass={activeFeatureClass} map={map} />}
