...
const ActiveClassControls = (props: { activeFeatureClass: IFeatureClass, map: Map }) => {
  ...
  const handleOpacityChange = (value: [number] | number) => {
    ...
    const activeFeatures = annotationSource.getFeatures().filter((feature) => feature.get("class") === activeFeatureClass.id)
    ...
    activeFeatureClass.style.fill.color = [...activeFeatureClass.style.fill.color.slice(0,3), newOpacity]
    const previousFeatureClasses = map.get('featureClasses')
    previousFeatureClasses[activeFeatureClass.id].style.fill.color = [...activeFeatureClass.style.fill.color.slice(0,3), newOpacity]
